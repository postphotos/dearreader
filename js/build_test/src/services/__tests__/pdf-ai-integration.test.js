...
describe('PDF + AI Integration', () => {
    const hasAnyAI = config.ai_providers?.openai?.api_key ||
        config.ai_providers?.openrouter?.api_key ||
        config.ai_providers?.gemini?.api_key;
    before(function () {
        if (!hasAnyAI) {
            console.log('⚠️  No AI providers configured, skipping integration tests');
            this.skip();
        }
    });
    ...
    describe('Multi-Provider AI Testing', () => {
        it('should test all available AI providers with same content', async function () {
            ...
            const successfulProviders = Object.values(results).filter(result => !result.startsWith('Error:') && !result.startsWith('Rate limited'));
