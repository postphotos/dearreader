...
describe('PDF + AI Integration', () => {
    ...
    const hasAnyAI = aiEnabled && config.ai_providers && Object.values(config.ai_providers).some((provider) => provider.api_key && provider.api_key !== '' && provider.api_key !== '${OPENROUTER_API_KEY}');
    ...
    if (hasAnyAI) {
        ...
        describe('Multi-Provider AI Testing', () => {
            it('should test all available AI providers with same content', async function () {
                ...
                const successfulProviders = Object.values(results).filter(result => !result.startsWith('Error:') && !result.startsWith('Rate limited') && !result.startsWith('Auth failed'));
