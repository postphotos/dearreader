...
describe('CrawlerHost Robots Checker Edge Cases', () => {
    it('should proceed when robotsChecker is missing methods', async () => {
        ...
        { scrape: async function* (_) { yield { title: 'X', href: 'http://example/', html: '<html></html>', text: 'X', parsed: { title: 'X', content: 'X', excerpt: 'X' }, imgs: [] }; }, _sn: 'mock', browser: null, logger: console, startTime: Date.now(), circuitBreakerHosts: new Set(), on: () => { }, emit: () => { } }, { inferSnapshot: (s) => s, narrowSnapshot: (s) => s, runTurndown: () => 'ok', snippetToElement: (snippet) => ({ querySelectorAll: () => [], innerHTML: snippet || '', textContent: snippet || '' }) }, { extract: async () => ({ content: 'pdf' }) }, 
